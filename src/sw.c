...
/// @return The current timeofday in microseconds, use this as the prev arg for
/// `sw_stop_us()`
...
inline double sw_stop_us(const double prev) { return (sw_start_us() - prev); }
...
/// @return The current timeofday in milliseconds, use this as the prev arg for
/// `sw_stop_ms()'
inline double sw_start_ms() { return sw_start_us() / 1000.0; }
...
inline double sw_stop_ms(const double prev) { return (sw_start_ms() - prev); }
...
/// @return The current timeofday in seconds, use this as the prev arg for
/// `sw_stop_s`
inline double sw_start_s() { return sw_start_ms() / 1000; }
...
inline double sw_stop_s(const double prev) { return (sw_start_s() - prev); }
...
/// @return The current timeofday in seconds, use this as the prev arg for
/// `sw_stop_min`
inline double sw_start_min() { return sw_start_s() / 60; }

/// Stop the second timer
/// @param prev The start time from `inline double sw_start_min()`
/// @return The time between stop and start in minutes
inline double sw_stop_min(const double prev) { return (sw_start_min() - prev); }
...
inline double sw_memory_size_b() {
    ...
    if (GetProcessMemoryInfo(
            hproc, (PPROCESS_MEMORY_COUNTERS)&pmc, sizeof(pmc)
        )) {
        // In bytes?
...
inline void sw_memory_print_b() { printf(PRINT_B_FORMAT, sw_memory_size_b()); }
#endif  // SW_PRINT_FUNCTIONS

/// Get the current memory usage of this application
/// @return The size of memory in kilobytes
inline double sw_memory_size_kb() { return sw_memory_size_b() / 1000.0; }
...
inline double sw_memory_size_mb() { return sw_memory_size_kb() / 1000.0; }
...
inline double sw_memory_size_gb() { return sw_memory_size_mb() / 1000.0; }
