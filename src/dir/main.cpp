...
double renderToTextureTimeSum;      // elapsed time for render-to-texture
double bindTimeSum;
double attachTimeSum;
double frameCount;
...
bool initSharedMem()
{
    ...
    renderToTextureTimeSum = 0;
    bindTimeSum = attachTimeSum = 0;
    frameCount = 0;
...
void showInfo()
{
    ...
    ss << "Render-To-Texture Time: " << (renderToTextureTimeSum / frameCount) << " us" << ends;
    ...
    {
        ss << std::fixed << std::setprecision(3);
        ss << "Bind Overhead: " << (bindTimeSum / frameCount) << " us" << ends;
        drawString(ss.str().c_str(), 1, screenHeight-(4*TEXT_HEIGHT), color, font);
        ss.str("");

        ss << std::fixed << std::setprecision(3);
        ss << "Attach Overhead: " << (attachTimeSum / frameCount) << " us" << ends;
...
void displayCB()
{
    ...
    //@@ remove all glFinish() calls if you don't measure the elapsed time
    glFinish();     // finish any pending GL calls before measuring elapsed time
    ...
    if(fboUsed)
    {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, fboId);
        glFinish(); // wait until the gl call is completed
        bindTimeSum += t2.getElapsedTimeInMicroSec(); // measure bind call overhead

        // measure function call overhead for attaching color-buffer image
        // NOTE: No need re-attach the texture to colour attchment every frame
        //       (Attach it only once when FBO is initialized)
        //       However, the following line is simply to measure the elapsed time
        //       and to compare it with FBO binding time.
        //       (Attaching texture should be faster than binding FBO)
        t2.start();
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 0, 0);          // detach
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureId, 0);  // attach
        glFinish(); // wait until GL call is completed
        attachTimeSum += t2.getElapsedTimeInMicroSec();
    ...
    else
    {
        ...
        glGenerateMipmap(GL_TEXTURE_2D);    //@@ fix for intel chipset bug: intel driver does not generate mipmap automatically
        glBindTexture(GL_TEXTURE_2D, 0);
    ...
    glFinish();     // finish any pending GL calls before measuring elapsed time
    t1.stop();
    renderToTextureTimeSum += t1.getElapsedTimeInMicroSec();
    ///////////////////////////////////////////////////////////////////////////
    ...
    // reset every 3 sec
    if(renderToTextureTimeSum > 3000000.0)
    {
        frameCount = 0;
        renderToTextureTimeSum = bindTimeSum = attachTimeSum = 0;
    }
}
